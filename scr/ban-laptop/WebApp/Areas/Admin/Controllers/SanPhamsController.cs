        ...
        public async Task<IActionResult> Edit(int id, [Bind("Ma,SN,TenSP,NgaySX,Hinh,MoTa,Gia,TrangThai,Dophangiai,Ram,Cpu,Content,Color,MaLoai,MaThuonghieu")] SanPham sanPham, IFormFile ful)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    {
                        // tạo tên file mới
                        var extension = Path.GetExtension(ful.FileName); // ví dụ: ".jpg"
                        var fileName = sanPham.Ma + extension;

                        // cập nhật lại tên hình vào database
                        sanPham.Hinh = fileName;

                        // tạo đường dẫn lưu ảnh
                        var path = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot/img/pro", fileName);
